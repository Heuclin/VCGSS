...
#' Main function to run the varying coefficient model using spike-and-slab prior for variable selection
#'
#' @param Y n x T matrix of phenotypic observation (n is the number of individuals, T is the number of observed time points)
#' @param X n x q matrix of variables (markers), q id the number of variables
#' @param ENV n x ne matrix of environmental variables, ne is the number of environmental varialbes
#' @param selection boolean indicating if selection is needed, the default is TRUE
...
#' @param niter number of iteration of the MCMC chain, the default is 10000
...
#' @param rep number of repetition, the default is one
...
#' Gelman Rubin's convergence diagnostic of the MCMC chain generated by the \code{VCM_fct} function
#'
#' @param object list generated by the \code{VCM_fct} function 
#' @param gelman.plot boolean indicating if the gelman plot should be plotted in the case where K is upper than one, used only if gelman.diag = TRUE, the default is FALSE
#' @param traceplot boolean indicating if the traceplot should be plotted in the case where K is upper than one, used only if gelman.diag = TRUE, the default is FALSE
#'
#' @return return input object including Gelman Rubin's convergence diagnostic 
#' @export
#'
#' @examples
...
#' Function to calculate parameter estimations from many MCMC chains generated by the \code{VCM_fct} function 
#'
#' @param object list generated by the \code{VCM_fct} function 
#'
#' @return returne the input object including the parameter estimations
#' @export
#'
#' @examples
...
#' Function to plot the effect curve 
#'
#' @param object list generated by the \code{VCM_fct} function 
#' @param mfrow A vector of the form c(nr, nc). Subsequent figures will be drawn in an nr-by-nc array on the device by rows. The default is c(6, 7).
...
#' @param id variable idinces for which you want to plot the effect curves. The default is indices of variables which have posterior marginal probabilities upper than 0.5
...
#'
#' @examples
plot_curve <- function(object, mfrow = c(6, 7), mar = c(1, 1, 4, 1), plot = c("Y", "mu", "env", "beta"), 
...
#' Function to plot visual convergence diagnostic of variable marginal posterior inclusion probabilities
#'
#' @param object  list generated by the \code{VCM_fct} function 
#'
#' @return
